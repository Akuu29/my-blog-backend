    ...
    U: ITagRepository,
{
    ...
    pub async fn create(
        ...
        Ok(self.article_repository.create(user_id, payload).await?)
    }
    ...
    ) -> Result<(Vec<Article>, ItemCount), UsecaseError> {
        Ok(self
            .article_repository
    ...
    pub async fn update_with_auth(
        ...
        Ok(self.article_repository.update(article_id, payload).await?)
    }
